...
if [[ $PRODUCTION == "yes"  ]]; then
	...
	if [[ $CONTAINER_NAME != "default" ]]; then
		docker run -it --workdir=/home --name=$CONTAINER_NAME --expose 80 --expose 3306 -p $PORT_APP:80 -p $PORT_DB:3306 dockercontainervm/collabtive:3.1 bash
	else
		docker run -it --workdir=/home --expose 80 --expose 3306 -p $PORT_APP:80 -p $PORT_DB:3306 dockercontainervm/collabtive:3.1 bash
	fi
fi
