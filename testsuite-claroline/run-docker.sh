...
if [[ $PRODUCTION == "yes"  ]]; then
	...
	docker run -it --workdir=/home/claroline --name=$CONTAINER_NAME --expose 80 --expose 3306 -p $PORT_APP:80 -p $PORT_DB:3306 -d --entrypoint ./run-services-docker.sh dockercontainervm/claroline:1.11.10 bash
else
	if [[ $CONTAINER_NAME != "default" ]]; then
		docker run -it --workdir=/home/claroline --name=$CONTAINER_NAME --expose 80 --expose 3306 -p $PORT_APP:80 -p $PORT_DB:3306 dockercontainervm/claroline:1.11.10 bash
	else
		docker run -it --workdir=/home/claroline --expose 80 --expose 3306 -p $PORT_APP:80 -p $PORT_DB:3306 dockercontainervm/claroline:1.11.10 bash
	fi
fi
